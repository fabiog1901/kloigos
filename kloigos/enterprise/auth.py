...
def _safe_json_loads(value: str | None, *, default: dict[str, str] | None = None) -> dict[str, str]:
    if not value:
        return default or {}
...
class OIDCConfig:
    enabled: bool = field(default_factory=lambda: _as_bool(os.getenv("OIDC_ENABLED"), default=False))
    enterprise_license_key: str = field(default_factory=_license_key_value)
    issuer_url: str = field(default_factory=lambda: os.getenv("OIDC_ISSUER_URL", "").strip().rstrip("/"))
    client_id: str = field(default_factory=lambda: os.getenv("OIDC_CLIENT_ID", "").strip())
    client_secret: str = field(default_factory=lambda: os.getenv("OIDC_CLIENT_SECRET", "").strip())
    scopes: str = field(default_factory=lambda: os.getenv("OIDC_SCOPES", "openid profile email").strip())
    audience: str = field(default_factory=lambda: os.getenv("OIDC_AUDIENCE", "").strip())
    extra_auth_params_raw: str = field(default_factory=lambda: os.getenv("OIDC_EXTRA_AUTH_PARAMS", "{}"))
    redirect_uri: str = field(default_factory=lambda: os.getenv("OIDC_REDIRECT_URI", "").strip())
    login_path: str = field(default_factory=lambda: os.getenv("OIDC_LOGIN_PATH", "/api/auth/login").strip())
    session_cookie_name: str = field(
        default_factory=lambda: os.getenv("OIDC_SESSION_COOKIE_NAME", "kloigos_session").strip()
    )
    state_cookie_name: str = field(
        default_factory=lambda: os.getenv("OIDC_STATE_COOKIE_NAME", "kloigos_oidc_state").strip()
    )
    nonce_cookie_name: str = field(
        default_factory=lambda: os.getenv("OIDC_NONCE_COOKIE_NAME", "kloigos_oidc_nonce").strip()
    )
    next_cookie_name: str = field(
        default_factory=lambda: os.getenv("OIDC_NEXT_COOKIE_NAME", "kloigos_oidc_next").strip()
    )
    cookie_secure: bool = field(default_factory=_cookie_secure_default)
    cookie_samesite: str = field(default_factory=lambda: os.getenv("OIDC_COOKIE_SAMESITE", "lax").strip().lower())
    cookie_domain: str | None = field(default_factory=lambda: os.getenv("OIDC_COOKIE_DOMAIN"))
    verify_audience: bool = field(default_factory=lambda: _as_bool(os.getenv("OIDC_VERIFY_AUDIENCE"), default=False))
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise RuntimeError(f"OIDC is enabled but missing required env vars: {', '.join(missing)}")

        if self.cookie_samesite not in {"lax", "strict", "none"}:
            raise RuntimeError("OIDC_COOKIE_SAMESITE must be one of: lax, strict, none")

        if self.cookie_samesite == "none" and not self.cookie_secure:
            raise RuntimeError("OIDC_COOKIE_SECURE must be true when OIDC_COOKIE_SAMESITE=none")
...
class OIDCManager:
    ...
    def get_metadata(self) -> dict[str, Any]:
        if self._metadata and (time.time() - self._meta_loaded_at) < self._cache_ttl_seconds:
    ...
    def get_jwks(self) -> dict[str, Any]:
        if self._jwks and (time.time() - self._jwks_loaded_at) < self._cache_ttl_seconds:
            return self._jwks
    ...
    def build_authorization_url(self, redirect_uri: str, state: str, nonce: str) -> str:
        ...
        if not auth_endpoint:
            raise RuntimeError("OIDC provider metadata missing 'authorization_endpoint'")
    ...
    def _select_jwk(self, token: str) -> Any:
        ...
        raise HTTPException(status_code=401, detail="Unable to find a matching JWKS key for token")

    def validate_jwt(self, token: str, *, expected_nonce: str | None = None, strict_client_audience: bool = False) -> dict[str, Any]:
        ...
        options = {
            ...
            "verify_nbf": True,
            "verify_iss": True,
            "verify_aud": strict_client_audience or self.config.verify_audience or bool(self.config.audience),
        }
    ...
    def current_claims(self, request: Request) -> dict[str, Any]:
        if not self.enabled:
            logging.warning("UNAUTHENTICATED!")
        ...
        if session_token:
            return self.validate_jwt(session_token)
...
def require_authenticated(request: Request) -> dict[str, Any]:
    return oidc.current_claims(request)
...
def oidc_login(request: Request, next: str = "/"):  # noqa: A002
    ...
    resp.set_cookie(oidc.config.next_cookie_name, next_path, max_age=300, **cookie_kwargs)
    return resp
...
def oidc_callback(
    ...
    if not id_token or not isinstance(id_token, str):
        raise HTTPException(status_code=401, detail="Token endpoint response missing id_token.")
    ...
    resp.set_cookie(oidc.config.session_cookie_name, id_token, max_age=3600, **cookie_kwargs)
    resp.delete_cookie(oidc.config.state_cookie_name, path="/", domain=oidc.config.cookie_domain)
    resp.delete_cookie(oidc.config.nonce_cookie_name, path="/", domain=oidc.config.cookie_domain)
    resp.delete_cookie(oidc.config.next_cookie_name, path="/", domain=oidc.config.cookie_domain)
    return resp
...
def oidc_logout():
    ...
    resp.delete_cookie(oidc.config.session_cookie_name, path="/", domain=oidc.config.cookie_domain)
    return resp


@router.get("/me")
def oidc_me(claims: dict[str, Any] = Depends(require_authenticated)) -> dict[str, Any]:
    return claims
